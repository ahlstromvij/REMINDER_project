...
fontsize: 12pt
title: |
  Does general political knowledge indicate issue-specific knowledge? 
  Stress-testing the generalist assumption
author:
  - name: 
    email: 
    corresponding_author:  #Footnote using corresponding_author
  - name: 
    email: 
footnote:
  - code: 
...
abstract: |
 \singlespacing Political knowledge scholarship tends to invoke what we call a *generalist assumption*: general knowledge indicates unmeasured, issue-specific knowledge. Yet the evidence for this assumption displays limitations of sample coverage and recency. Is it warranted? Using convergent evidence from best-available U.S. and European surveys (total N=82,919), we argue it is while advising against deploying it uncritically. Factor analysis of European data that, unusually, includes items about general politics and the specific issue of EU immigration reveals plausible unidimensionality. Then, knowledge scales built using Item Response Theory modeling display similar associations with known correlates of political knowledge -- gender, age, education -- mirrored in ANES, CES, and BES measures. These patterns remain after examining issue-specific knowledge about COVID-19 in the U.K., although knowledge about climate change in the U.K., Germany, Norway, and France exhibits some variation by age and gender. We conclude with practical guidance and reproducible code for stress-testing unidimensionality assumptions.
 \center __Word count__: 8,230 words
acknowledgements: |
  Removed for anonymized reviewing.
keywords:
  - political knowledge
  - immigration
  - public health
...
knitr::opts_chunk$set(echo = FALSE, warning = FALSE) # By default, hide code and warnings; set to TRUE to see code
...
The idea that people who are knowledgeable in one area of politics will be knowledgeable in others has underpinned much of the key work in public opinion [@zaller1986; @zaller1992;@carpinikeeter1993; @carpinikeeter1996]. Methodologically, this is attractive because it implies that researchers can use a general knowledge scale to measure the extent to which people are politically informed across specific domains, even in the absence of items directly concerned with the particular topics represented within those domains. Yet despite its methodological and practical appeal, this assumption has remained largely untested over the past 30 years [@carpinikeeter1996; @burnett_mccubbins_2020], partly due to the lack of available survey instruments which contain both general and issue-specific factual items. Moreover, the evidence that does exist is limited to relatively small-scale samples exclusively drawn from the U.S.

In response, we present convergent evidence that updates @carpinikeeter1996's classic investigation by stress-testing the generalist assumption. We do so by applying a variety of methods to a range of the best-available survey evidence spanning geographic contexts (the U.S. and seven European countries), topics (migration, public health, and climate change), and sampling types. Among other steps, we exploit cross-national European survey data that, unusually, contains questions about both general politics and a specific issue, namely European Union immigration [@meltzer2020]. By assessing the dimensionality and construct validity of the full range of knowledge questions contained within our assembled surveys, and by comparing these features to widely-used instruments -- the American National Elections Studies, the Cooperative Election Study, and the British Election Study -- we conclude that the generalist assumption holds up to scrutiny. At the same time, in extending our analysis to issue-specific knowledge about COVID-19 and climate change, we uncover variations in how the latter scale correlates with gender and age that highlight how the assumption should not be deployed uncritically. 

Therefore, our study makes two contributions that are relevant for broader political scientific inquiry. First, we provide up-to-date, larger-scale, and comparative empirical evidence about the (non)uniqueness of general political knowledge as actually measured in readily-available surveys. Second, we offer practical advice for researchers who either want to conduct similar stress-tests of their own topic- and context-specific survey data on grounds of unidimensionality assumptions, or are considering which types of knowledge questions to include in future studies while acknowledging concerns about resources, time, and survey attentiveness. To that end, and in line with Open Science principles, we also provide the _R_ code underlying the present investigation in a public GitHub repository, as a reproducible template for others to use.[^1]

[^1]: \doublespacing Link removed for anonymized reviewing, but can be provided if required at a later stage.

# The Generalist Assumption about Political Knowledge

Our main contention in this section is that the generalist assumption has come to carry an increasingly heavy burden in political scientific scholarship on knowledge. This is understandable, given that making such an assumption widens the data sets that researchers can use, and thereby drives empirical progress. However, since there have only been two systematic investigations into whether the assumption actually holds over the last three decades [@carpinikeeter1996; @burnett_mccubbins_2020] -- neither of which consider contexts beyond the U.S. -- another investigation is overdue. In this section, we elaborate on this narrative while clarifying the distinction between general and issue-specific knowledge.

## Generalist versus Specific Knowledge: Conceptual Issues {#conceptual-issues}

The generalist assumption can be traced back at least to John Zaller's analysis of information items in the 1985 National Election Study (NES) Pilot, where he argued that "political information is a relatively general trait that can be effectively measured with a general-purpose information scale" [@zaller1986, 2]. This conclusion subsequently informed his landmark study on public opinion, where he declared that he was "assuming that persons who are knowledgeable about politics in general are habitually attentive to communications on most particular issues as well" [@zaller1992, 43]. @carpinikeeter1996 investigated and defended that assumption, which later featured in both @bartels1996 and @althaus2003's highly influential work on "information effects," i.e. the extent to which voters would have expressed different preferences had they possessed more information.

What, then, does it mean to be knowledgeable about politics in general, as Zaller puts it? It is helpful to follow @carpinikeeter1996 in conceptualizing political knowledge as a *resource*: having a lot of it means being in a position to navigate the political world, and to stand a better chance of connecting your fundamental political goals with successful means--in the simplest case, by voting for the candidate or party that is best place to realize those goals (see also @zaller1992 on how the politically informed tend to be the most ideologically coherent). It is against this background that Delli Carpini and Keeter identify (general) political knowledge as comprising
...
Knowledge of these areas is "critical to the maintenance of a healthy democracy" [@carpinikeeter1996, 14], on account of how a person with such knowledge inhabits the position of a (more) effective political actor, in the minimal case as a voter.[^2] That said, considering @carpinikeeter1993's recommended five-item index (304-5), the emphasis is very much on the the rules of the game and the main political actors--on "what the government is and does", as they put it. In their favored index, the component items ask (1) which party controls the House of Representatives; (2) the percent of the vote required in Congress to override a presidential veto; (3) where the two U.S. parties are located on the political spectrum; (4) which branch of government determines the constitutionality of laws; and (5) who is the U.S. Vice President. 

[^2]: \doublespacing There are important debates about the limitations of this conception of political knowledge. These include observations of how voters often use their lived experiences alongside facts to interpret and engage with political processes [@cramer2017]; and how the formulation of knowledge questions themselves may disadvantage racial and ethnic minorities [@abrajano2015] and women [@kraft2022] by prioritizing some areas of knowledge over others. We note two points here. First, the notion of what we might think of as *technical* political knowledge at work here does not rule out other types of *situated* forms of knowledge--e.g. knowing what it is like to go hungry for days on end, or to be on the receiving end of racism--that might enable political agents to not only pursue political goals effectively but also to choose morally appropriate ends. Second, if the type of knowledge necessary for effective political participation disadvantages the marginalized, the response should not be to reject such knowledge, but to expand its reach, and thereby empower a larger section of society to take meaningful political action.

As we will explain later (section \ref{construct-validation-2}), this conception of general political knowledge as being primarily about knowing how government works and what the main political actors are coheres well with the content of knowledge items we tend to see in established data sets such as the American National Elections Studies (ANES), the Cooperative Election Study (CES), and the British Election Study (BES). In the case of the ANES, this is no surprise: Delli Carpini and Keeter's work fed directly into the knowledge items in that survey, which in turn set the standard for other data sets within and outside of the U.S. By contrast, the type of items that we will consider in relation to immigration, public health (COVID-19), and climate change might get to important issues that people care deeply about, but do not pertain directly to the central resource that prior literature conventionally calls "general" political knowledge.

A cynic might read this separating out of the "substance of politics" from general knowledge proper as a *post hoc* attempt to categorize established knowledge items and scales as pertaining to general political knowledge. However, there is independent reason for such a separation, given the asymmetry between general and issue-specific knowledge when it comes to putting knowledge into action. While being a generally effective political actor who knows how the system works and who the main players are is necessary (but perhaps not sufficient) for making progress on specific, substantive policy issues, the reverse does not hold. That is, while it might be beneficial to be highly informed on specific issues in order to be a well-motivated and effective political actor, as the literature on issue publics suggests [@kim2009; @henderson2014], it is not strictly speaking necessary.
...
To evaluate the core elements of the generalist assumption, we apply a four-step approach that expands on that taken by @carpinikeeter1996 in their aforementioned, early work on knowledge scales. 
...
[^4]: \doublespacing We do not look at scale reliability as measured by Chronbach's alpha, since such reliability can be high even given multidimensionality [@fabrigar2011].
...
kable(fa_tibble, 
      "latex", 
      booktabs = T, 
      caption="EFA of general (G) and immigration (I) knowledge items", 
      label="tab2")
```
...
kable(fit_measures, 
      "latex", 
      booktabs = T,
      caption="CFA fit measures of general (G) and immigration (I) knowledge items for unidimensional (1D) and two-dimensional (2D) models", 
      label="tab3")
```
...
## Step 2: Construct Validation Through Known Demographic Correlates {#construct-validation-1}
...
To do this, we first create two knowledge scales using IRT modeling [@deayala2009; @demars2010]. IRT models measure latent traits assumed to fall on a continuous scale. Values on that scale are usually referred to by way of $\theta$ (theta), and taken to range from -$\infty$ to +$\infty$, with a mean of 0 and standard deviation of 1. This means that, while the individual $\theta$ value ascribed to any particular respondent has no intrinsic meaning, it can nevertheless be interpreted relative to an estimated population mean. Full details on the IRT models used in this section as well as those used subsequently can be found in the Supporting Information.[^5] 

[^5]: \doublespacing We account for the possibility of guessing by fitting separate three-parameter models for the general and immigration-specific knowledge scales (see Supporting Information). These results indicate a very low likelihood of guessing.

Having ensured that the two sets of items satisfy standard assumptions for an IRT model (unidimensionality, local independence, and acceptable model fit), we then regress these scales on gender, age, and education, across the geographically pooled REMINDER data set (i.e. not segmenting by nationality). The first two variables are self-explanatory. As mentioned above, the education variable takes on a value from 0 to 8, corresponding to the nine ISCED levels of education, from early childhood education (0) to doctorate (8). Details about the resulting models can be found in Table \ref{tab:4}.
...
texreg(list(m_gen, m_imm),
               single.row = TRUE,
               caption = "Construct validity regression models (OLS)",
...
Table \ref{tab:4} shows how the coefficient values in both cases cohere with what we know about the relationship between political knowledge and demographics: men tend to have more political knowledge than women [@vanheerdehudson2020; @plutzer2020; @carpinikeeter1996], and the educated more political knowledge than the uneducated [@hebbelstrup2016]. This echoes the patterns found by @barabas2014 in their study of knowledge items. It also seems reasonable to assume that political knowledge increases with age [@stockemer2017], as indeed is the case on both models. 

Moreover, as can also be seen from Table \ref{tab:4}, the associations are very similar between the two scales, which speaks in favor of unidimensionality: both scales appear to be measuring the same fundamental, underlying trait. We can illustrate the similarity between the two scales further by looking at the mean levels of knowledge across aforementioned demographics. We see in Figure \ref{fig:violinplots} that the mean level of knowledge is very similar between the two scales for the different levels/categories. 

```{r violinplots, fig.cap = "Mean levels of knowledge (solid bars) across gender, age, and education", echo=FALSE, fig.width=11, fig.height=4}
long_df <- reminder_data %>% 
...
texreg(list(m_knowledge),
               single.row = TRUE,
               caption = "Knowledge type regression model (OLS)",
...
We can probe these similarities more formally. Looking at Figure \ref{fig:violinplots}, we would expect that a fair amount of differences between levels of knowledge would be accounted for by the three demographic variables. Once we factor in what is accounted for by these variables, do any significant differences remain between the two types of knowledge? To answer this question, we pool all knowledge values, irrespective of type, and then fit a model with those pooled values as the dependent variable, and knowledge type (general or immigration), gender, age, and education as independent variables. If any significant differences remain between the two types of knowledge, once we have factored in the demographic variables, we should expect a significant coefficient on the knowledge type variable. As can be seen from Table \ref{tab:5}, that is not what we find. The coefficient value is not significant (_p_ = `r round(summary(m_knowledge)$coefficients[2,4],4)`), and moreover minimal. As a result, it is no surprise that the coefficient values for our demographic variables in Table \ref{tab:5} are also very similar to those in Table \ref{tab:4}. This suggests that, once we factor in those variables, there is no variance left for our knowledge type variable to explain. This is, of course, exactly what we should expect if the two scales both tap into the same underlying phenomenon of political knowledge.

## Step 3: Construct Validation Through Similarity with Established Scales {#construct-validation-2}

As a further test of construct validity, we compare our two scales with other widely-used measures of political knowledge from the American National Elections Study (ANES), the Cooperative Election Study (CES), and the British Election Study (BES). All of the question batteries we report here form knowledge scales that satisfy standard IRT modeling assumptions (see Supporting Information for details). The ANES data set is the 2019 Pilot Study [@anes2019] (N = 3,000), which contains the following knowledge items:
...
Finally, in the case of BES, we use the 2017 Post-Election Face-to-Face Survey [@fieldhouse2018] (N = 2,067)[^6], which contains the following knowledge items:
...
[^6]: \doublespacing The original data set contains 2,194 observations. 127 observations with missing survey weights were removed, for a resulting sample size of 2,067.

These questions are valuable for our stress-testing objective because, while all general in the sense discussed in section \ref{conceptual-issues}, they vary in terms of their temporal dimension [@barabas2014]. For example, some items ask about facts regarding well-established political processes or features (e.g. ANES3, BES1-4), while others require a degree of  surveillance on the part of respondents (e.g. ANES1, ANES2, CES1-4). Considering these differences matters because they mirror theoretical expectations of how knowledge informs political behavior. On the one hand, knowledge of fundamental and unchanging political rules potentially enables citizens to actively participate in politics [@carpinikeeter1996]. On the other hand, knowing more recent facts potentially signals an ability to learn about -- and develop views on -- political events as they happen [@schudson2011].
...
emmeans_age_cat <- emmeans_age_cat_combined %>% 
  ggplot() +
  aes(x = age_cat, y = emmean, linetype = Knowledge, shape = Knowledge, group = Knowledge) +
  geom_line() +
  geom_pointrange(aes(ymin=lower.CL, ymax=upper.CL)) +
  geom_hline(yintercept=0, linetype="dashed", alpha=0.5) +
  xlab("Age") +
...
emmeans_education_ISCED <- emmeans_education_ISCED_combined %>% 
  ggplot() +
  aes(x = education_ISCED, y = emmean, linetype = Knowledge, shape = Knowledge, group = Knowledge) +
  geom_line() +
  geom_pointrange(aes(ymin=lower.CL, ymax=upper.CL)) +
  geom_hline(yintercept=0, linetype="dashed", alpha=0.5) +
  xlab("Education") +
...
```{r emmeans_plots2, fig.cap = "Estimated marginal means for ANES, CES, and BES knowledge scales", echo=FALSE, fig.width=11,fig.height=10.5}
...
Finally, as shown in Figure \ref{fig:emmeans_plots3}, breaking down the estimated means from the REMINDER scales by country suggests that the observations made about the geographically pooled data appear to hold in the cases of individual countries, too. This offers further evidence that the generalist assumption is robust in the face of geographic segmentation.

```{r emmeans_plots3, fig.cap = "Estimated marginal means for general (solid line) and immigration (dashed line) knowledge scale by country", echo=FALSE, fig.width=11,fig.height=16}
...
means_by_nationality <- function(nationalities, knowledge_gen, knowledge_imm) {
  ...
  for (i in 1:length(nationalities)) {
    ...
    plot <- ggarrange(emmeans_education_ISCED, emmeans_gender, emmeans_age_cat, nrow = 1, common.legend = TRUE, legend = FALSE)
...
## Step 4: Extending the Generalist Assumption beyond Migration
...
These topics, and the questions within them, are useful for stress-testing the generality of our results from the previous section because, while they both deal with policy-specific domains, they vary in terms of temporal dimension just as the previous general knowledge questions from the ANES, CES, and BES do [@barabas2014]. Facts about climate change have arguably become well-established in public consciousness, whereas possessing knowledge about COVID-19 -- at the time of the survey fieldwork -- would likely indicate a degree of active surveillance by respondents.

Our COVID-19 data set comes from a pre-registered online survey experiment (N = 2,917) that we had fielded in the U.K. in July 2020 after the country's first pandemic wave.[^7] The sample was provided by Prolific.ac, which is a widely-used platform that performs well on several quality benchmarks including respondent attention and honesty [@peer2022]. For our purposes, we use the following knowledge items, measured pre-treatment, which together form a scale satisfying standard assumptions for IRT (again, see the Supporting Information document for further details on the IRT models):

[^7]: \doublespacing The survey received IRB approval from the relevant university committee. To preserve anonymity, documentation of this approval is withheld at the review stage, but can be provided upon request.
...
The climate change data set is from the "European Perceptions of Climate Change" survey [@pidgeon2016] that was fielded in the U.K., France, Germany, and Norway by the polling company Ipsos MORI through its face-to-face omnibus service in June 2016 (N = 4,048). The survey was not specifically designed to measure knowledge about climate change, but it does contain three items which could reasonably be taken to tap into such knowledge,[^8] and moreover form a scale satisfying standard assumptions for IRT:

[^8]: \doublespacing As (CC2) is formulated in terms of the respondent's opinion, it might be taken not to measure someone's level of knowledge. However, as the item asks about the respondent's opinion on a matter of fact, and any opinion taken by the respondent thereby can be deemed correct or incorrect, we treat it as a knowledge item.

> (CC1) As far as you know, do you think the world's climate is changing or not? (Correct: The climate is changing)\
> (CC2) Thinking about the causes of climate change, which, if any, of the following best describes your opinion? (Correct: Climate change is mainly or completely caused by human activity)\
...
```{r emmeans_plots4, fig.cap = "Estimated marginal means for COVID knowledge scale", echo=FALSE, fig.width=11,fig.height=4}
emmeans_function(m_covid,
                 c("education","gender","age_cat"),
                 c("Highest level of education","Gender","Age bracket"))
```

Figure \ref{fig:emmeans_plots4} plots the estimated marginal means for the same demographic variables as in the previous section for the COVID knowledge scales. The marginal means by education shows the same, monotonic trend seen in Figure \ref{fig:emmeans_plots1} and \ref{fig:emmeans_plots2}. Men generally have more knowledge than women, and knowledge increases with age, which again aligns with what we found in regards to the five scales in the previous section.

```{r emmeans_plots5, fig.cap = "Estimated marginal means for climate change knowledge scale", echo=FALSE, fig.width=11,fig.height=15}
...
ggarrange(plotlist = list(annotate_figure(epcc_uk_plot, top = text_grob("U.K.", 
                                                                        color = "black", size = 16)),
...
Turning to Figure \ref{fig:emmeans_plots5}, the picture is more mixed. For each of the four countries (U.K., France, Germany, and Norway), we see the same type of trend in regards to education as in Figure \ref{fig:emmeans_plots1} and \ref{fig:emmeans_plots2}.[^9] Moreover, in three of the four countries, men tend to know more about climate change than women, with Germany being the exception to this rule. In the case of age, we see a reversal of the pattern of knowledge largely increasing with age in three of the four countries, with younger people tending to have more knowledge about climate change than older people in the U.K., France, and Norway. We return to the implications for the generalist assumption in the discussion section.

[^9]: \doublespacing In the interest of comparability, the education variables for France, Germany, and Norway were reclassified to conform with the education categories in the U.K. segment, as follows: For France, "No diploma" = No qualification; "Diploma under A-levels" = GCSE; "A-level" = A-level; and "High education" = Degree. For Germany, "Secondary school without completed vocational training" = No qualification; "Secondary school with completed vocational training" = GCSE; "Secondary school/grammar school /"O" level without diploma" = GCSE; "Secondary school/grammar school /"A" level with diploma" = A-level; and "University/technical university / polytechnic / technical college with degree" = Degree. For Norway, "Primary school" = No qualification; "Secondary school – without A-Level" = GCSE; "Secondary school – with A-Level" = A-level; "University Bachelor" or "University Postgraduate" = Degree.

# Discussion and Practical Guidance

A long line of research demonstrates how being more politically informed potentially matters for a host of outcomes, including what people think and prefer about issues. However, whether on principled or practical grounds, much of this work invokes what we have called the *generalist assumption*: that knowledge about general political topics is indicative of knowledge about issue-specific topics. From a measurement perspective, this assumption is useful because it allows researchers to deploy necessarily limited question batteries as proxies for other unmeasured forms of knowledge that are actually of interest. Substantively, it implies that political knowledge -- as conventionally measured in surveys -- may not be unique.

Yet despite its centrality to core public opinion work, this assumption is difficult to empirically test owing to the lack of surveys that contain general and specific knowledge questions. Moreover, while having evidence of whether and in which circumstances it holds would present implications for research that uses knowledge as an explanatory variable, early efforts by Delli Carpini and Keeter nearly 30 years ago to probe this assumption have been rarely updated with more up-to-date methods and data, with the exception of @burnett_mccubbins_2020. Even then, no study to the best of our knowledge has considered the degree to which it holds in contexts outside the United States.

In response, we have exploited a recent, large-scale survey that, unusually, contains both types of items in one instrument across seven European countries. Combining several analytical approaches, we conclude that the preponderance of evidence points to the generalist assumption being defensible. First, exploratory and confirmatory factor analyses applied to various combinations of the knowledge items revealed how the collection of questions performed well when treated unidimensionally. Second, in terms of construct validity, key demographic features that prior work leads us to expect are relevant to political knowledge exhibit similar associations between our political and immigration knowledge scales. Moreover, both scales in the aggregate and by country exhibit similar patterns in estimated marginal mean level of knowledge to established knowledge scales built from questions in the ANES, CES, and BES.

Our study makes two important contributions to understanding about the validity and specificity of general political knowledge questions as they are widely used in contemporary survey practice. First, we validate the generalist assumption of political knowledge using adult populations and up-to-date methods: knowing more about general politics is indicative of domain-specific knowledge. This echoes @burnett_mccubbins_2020's claim that political knowledge is not unique. However, without disagreeing with their conclusion that this feature raises important measurement questions about political knowledge, we also take the view that this feature lends support for the practical choice of inferring respondents' knowledge about specific issues from their general political knowledge levels. Second, we demonstrate how the generalist assumption appears to hold across a much wider range of geographic contexts and sampling methods than those tested by @carpinikeeter1996 and @burnett_mccubbins_2020. While we acknowledge our own investigation has been limited to European cases, we think this represents a significant advance on existing evidence and paves the way for further comparative work using a similar workflow. 

At the same time, moving beyond immigration knowledge as the test case for issue-specific knowledge, our investigation into two knowledge scales measuring knowledge about COVID-19 and climate change -- useful not the least since they reflect knowledge based in both longer- and shorter-term events -- make clear that the assumption should not be deployed uncritically. Specifically, while our COVID-19 scale exhibited very similar patterns across demographic variables as the scales derived from the REMINDER, ANES, CES, and BES data sets, the marginal mean levels of climate change knowledge revealed some geographic differences: in one case (Germany), women had more knowledge than men; and in three cases (U.K., France, and Norway), young people tended to have more knowledge than older people. This can be interpreted in either of two ways. On the one hand, we can take it to impugn the construct validity of the scale. As noted earlier, the items used to build the scale were not necessarily intended to be used as knowledge items by the researchers fielding the survey. On the other hand, knowledge about climate change might have a different age distribution given the potential for higher levels of awareness among younger compared to older people, in light of how the former have a greater number of years of exposure to the effects of climate change ahead of them. This would, of course, not explain the reversed gender effect in the case of Germany. 

Therefore, while our results taken together offer support for the generalist assumption -- that is, in many cases it is likely safe to assume, with Zaller, "that persons who are knowledgeable about politics in general are habitually attentive to communications on most particular issues as well" [@zaller1992, 43] -- we advise that researchers still stress-test the assumption as far as available data sets allow. This can be done, for example, by including knowledge items from different domains, and by using a combination of methods to evaluate its plausibility in specific contexts. Having evidence of this would be useful in at least two situations: where researchers want to evaluate the dimensionality of knowledge items available to them to decide which scales to use; and where researchers are collecting data, want to run a pilot with a range of knowledge items, and then choose the minimum number of items needed to achieve their objectives to potentially save funds and survey time.

In practical terms, we offer the following guidelines informed by our analysis:

1.  Start by conducting a parallel analysis on the total set of knowledge items, and then investigate the loadings from an exploratory factor analysis (EFA) on those same items, where the number of factors assumed in the EFA is informed by the parallel analysis. (See section \ref{efa-cfa}.)\
2.  Let the EFA results inform a confirmatory factor analysis (CFA) that compares a unidimensional model to one or several multidimensional models, depending on the number of factors suggested by the EFA, in order to see whether the former exhibits a good enough fit to compete with the multidimensional model(s). If it does, this would point towards unidimensionality. (See section \ref{efa-cfa}.)\
3.  Construct two or more knowledge scales, depending on the dimensionality under investigation, and then use each scale as an outcome in a regression model, with gender, education, and age as predictors. See if there any similarities in coefficient values between the two (or more) models. High similarity across all predictors would count towards unidimensionality. (See section \ref{construct-validation-1}.)\
4.  Pool the knowledge scores of all knowledge scales, and use those scores as a dependent variable in a regression model, with type of scale (e.g., putative type of knowledge) as an independent variable, alongside gender, education, and age. If the coefficient value of the variable representing the type of scale is small and not significant, that counts towards unidimensionality. (See section \ref{construct-validation-1}.)\
5.  Estimate marginal mean levels of knowledge by gender, education, and age, respectively, for each of the scales, and compare the patterns in these levels to those exhibited in Figures \ref{fig:emmeans_plots1}, \ref{fig:emmeans_plots2} and \ref{fig:emmeans_plots4}. Similarity with the patterns found there would count towards both construct validity and unidimensionality. (See section \ref{construct-validation-2}.)\

For any given project, all or only some of these steps will be appropriate or possible. However, it is our hope that the above steps will offer helpful guidance for researchers concerned with questions about knowledge scale dimensionality. To facilitate replication, and in line with Open Science principles, we also provide the _R_ code underlying the present investigation as a template for others to use in a public GitHub repository.[^10] 

[^10]: \doublespacing Link removed for anonymized reviewing.

# Statements of Data Availability and Competing Interests
The data that support the findings of this study are openly available at (link withheld for reviewing). The author(s) declare no competing interests.
